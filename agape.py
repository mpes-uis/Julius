...
def readData_Agape(url, assunto, prefeitura, readAgain):
    contador = 1
    pag_total = None
    while (contador is not None):
        ...
        try:
            ...
            if existing_data.empty:
                new_data.to_sql(assunto, conn, if_exists='replace', index=False)
            else:
            ...
            readAndSaveUrl(url_agape, assunto, prefeitura, contador, sucess, erro, None, pag_total) #salva a url e o respectivo erro
        ...
        readAndSaveUrl(url_agape, assunto, prefeitura, contador, sucess, erro, data['pagina_proxima'],pag_total) #salva a url lida com sucesso
...
def readAndSaveUrl(url_agape, assunto, prefeitura, contador, sucess, erro, proxima, pag_total):
    colunas = ['url','assunto','prefeitura', 'contador','sucess','erro','data_leitura','proxima','pag_total']
    leituras = pd.DataFrame(columns=colunas)
    registro={
         "url":url_agape,
...
def readData_Agape_Total():
    for id in prefeiturasURLS["id"]:
            for assunto in assuntos:
                        if prefeiturasURLS["empresa"][id]=="Agape" or prefeiturasURLS["empresa"][id]=="Alphatec" :
                                readData_Agape(prefeiturasURLS["url"][id], assunto, prefeiturasURLS["prefeitura"][id], False)

#readData_Agape_Total()
