gera_agape <- function(prefeituras_agape, assuntos_agape) {
  ...
  for (prefeitura in 1:nrow(prefeituras_agape)) {
    for (assunto in 1:nrow(assuntos_agape)) {
      ...
      response <- try(GET(url, timeout(5)), silent = TRUE)
        ...
        if (status_code(response) == 200) {
          ...
          for(pagina in 1:paginas) {
            ...
            if (resultado == 0) {
              ...
              df <- try(consulta_agape(prefeituras_agape[prefeitura, 4],
              ...
              if (class(df) == "data.frame"){
                ...
                cat("\033[1;35m", nome_tabela, "de", paginas, "demorou pra responder\n", "\033[0m")
              }
            }
            else {
